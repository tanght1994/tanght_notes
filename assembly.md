...
服务器方面，协程并不是为了快，因为单线程+多路IO复用比协程还快资源占用还少，我们使用协程是为了像同步编程一样来异步编程，避免复杂的回调函数

各种IO都用epoll监听，然后epoll返回的时候for处理所有事件，根本用不着协程，比如nginx，人家没用协程，照样快

如果你非要拿协程和线程对比，那么协程资源占用更少，切换速度更快

协程序切换核心代码

```
coctx_swap(当前协程上下文地址-A, 目标协程上下文地址-B)
协程上下文就是一个数组，用来保存协程上下文

#if defined(__i386__)
	// 保存当前协程的寄存器到 A 中
    movl 4(%esp), %eax     // 将eax设置为A的地址
    movl %esp,  28(%eax)   // 将寄存器的值保存到A[7] call指令会将指令指针入栈 所以现在的esp是函数的下一行代码
    movl %ebp, 24(%eax)    // 将寄存器的值保存到A[6]
    movl %esi, 20(%eax)    // 将寄存器的值保存到A[5]
    movl %edi, 16(%eax)    // 将寄存器的值保存到A[4]
    movl %edx, 12(%eax)    // 将寄存器的值保存到A[3]
    movl %ecx, 8(%eax)     // 将寄存器的值保存到A[2]
    movl %ebx, 4(%eax)     // 将寄存器的值保存到A[1]

	// 用 B 的数据填充各种寄存器, 以及设置下一行执行的代码为B[7]
    movl 8(%esp), %eax    // 将eax设置为B的地址
    movl 4(%eax), %ebx    // 将B[1]设置到寄存器
    movl 8(%eax), %ecx    // 将B[2]设置到寄存器
    movl 12(%eax), %edx   // 将B[3]设置到寄存器
    movl 16(%eax), %edi   // 将B[4]设置到寄存器
    movl 20(%eax), %esi   // 将B[5]设置到寄存器
    movl 24(%eax), %ebp   // 将B[6]设置到寄存器
    movl 28(%eax), %esp   // 将B[7]设置到寄存器  B[7]是指令指针

	ret  // ret会弹出栈。并将弹出的值设置到指令计数器 也就是说 将指令计数器设置为B[7]
```
