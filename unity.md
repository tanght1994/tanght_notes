...
返回x除以y的余数，对于正数来说，我们基本可以口算出来，对于负数来说，要用下面的公式计算

mod函数的底层原理为：x - y * floor(x / y)

```glsl
mod(x, y)
```

![image-20250225030618864](./assets/image-20250225030618864.png)
...
将x的值限制在a，b之间，如果x比a小，则返回a，如果x比b大，则返回b，如果x在a，b之间，则返回x
...
在x，y之间按照比例a进行插值，相当于其它语言中的lerp函数
...
在 edge0 和 edge1 之间进行平滑的插值，当 x 小于等于 edge0 时返回 0，大于等于 edge1 时返回 1，在中间则返回按三次插值函数计算的值（0-1之间）。为什么要按照三次插值进行计算，而不是直接用线性插值进行计算？为了保持边界处的导数为0，视觉效果更加自然。

不管如何设置edge0和edge1，函数返回值的范围都是[0-1]

edge0 > edge1和edge0 < edge1两种情况的图像正好相反

edge0 > edge1时的图像：

![image-20250225025121102](./assets/image-20250225025121102.png)

edge0 < edge1时的图像：

![image-20250225025039494](./assets/image-20250225025039494.png)

```glsl
smoothstep(edge0, edge1, x)
// 当 x <= edge0 时，返回 0.0
// 当 x >= edge1 时，返回 1.0
// 当 x 在 edge0 和 edge1 之间时，进行平滑插值
```
...
Kinematic的物体可以影响Dynamic物体，但是其它类型的物体（Static、Dynamic）影响不了Kinematic物体，Kinematic物体只受程序员的脚本控制。

当Kinematic物体碰到其它物体时，物理引擎不会处理碰撞，只会触发碰撞回调函数，需要程序员自己控制碰撞行为。
